...
Meteor.methods({
    swimmerUpdate: function(swimmerAttributes){
        ...
        console.log("dni: " + swimmerAttributes.dni);
        var result = nadadorConMismoDni(swimmerAttributes.dni);
        if(result.swimmerExist & result._id != swimmerAttributes._id){   
            console.log("otro con el mismo dni!!!!");
            return result;
        }
        
        result = nadadorConMismoEmail(swimmerAttributes.email);
        if(result.emailExist & result._id != swimmerAttributes._id){
            console.log("otro con el mismo email!!!!");
            return result;
        }
...
nadadorConMismoDni = function (dni) {
    console.log("entra1");
    var nadadorConMismoDni = Nadadores.findOne({dni: dni})
    if (nadadorConMismoDni) {
        ...
        console.log("no esta repetido dni");
        return{ swimmerExist: false };
    }
},
nadadorConMismoEmail = function (email) {
    console.log("entra2");
    var nadadorConMismoEmail = Nadadores.findOne({email: email});    
    if (nadadorConMismoEmail) {
        ...
        console.log("no esta repetido email");
