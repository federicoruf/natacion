...
Router.route('/swimmerArrive', {
  name: 'swimmerArrive',
  waitOn: function() {
    return [Meteor.subscribe('ordenDeLlegada')];
  }
});
...
Router.route('/usersAdministration', {
  ...
  waitOn: function() {           //primero
    return [Meteor.subscribe('users'), Meteor.subscribe('roles')];
  }
});
...
Router.onBeforeAction(requireDirectorRights, {only: ['categoryAdd', 
                                                    ...
                                                    'swimmerEdit',
                                                    'addSwimmer',
                                                    'usersAdministration']});  //AGREGAR LUEGO AQUELLOS RELACIONADOS AL ABM
